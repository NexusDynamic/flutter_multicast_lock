...
import 'package:flutter_multicast_lock_example/main.dart';

void main() {
  testWidgets('App starts with correct initial state', (
    WidgetTester tester,
  ) async {
  ...
  testWidgets('Buttons are tappable and trigger status updates on non-Android', (
    WidgetTester tester,
  ) async {
    await tester.pumpWidget(const MyApp());
    ...
    expect(
      find.text(
        'Status: Failed to acquire lock: Android multicast lock is only supported on Android platforms',
      ),
      findsOneWidget,
    );
    ...
    expect(
      find.text(
        'Status: Failed to check lock status: Android multicast lock is only supported on Android platforms',
      ),
      findsOneWidget,
    );
  });
